document.addEventListener('DOMContentLoaded', () => { // 讓HTML都載入後才跑DOM
    ...
    */
    ...
    // 以下13週作業新增內容
    const top10gameHttps = 'https://api.twitch.tv/kraken/games/top'; // 前十名遊戲API網址
    const twitchHeaderData = {
        'Client-ID': '64xr2r7xnpdra0dakqsvqryuzxkrd0',
        Accept: 'application/vnd.twitchtv.v5+json',
    };

    function fetchGetTwitchAPI(httpsAPI) { // 使用fetch獲取Twitch資料
        return fetch(httpsAPI, {
            method: 'GET',
            headers: new Headers(twitchHeaderData),
        }).then(response => response.json())
            .catch(() => '系統不穩定，請再試一次');
    }

    // 為直播區增加直播box
    async function addChannelsList(gameName) {
        channelsList.innerHTML = ''; // 清空直播區
        channelsTitle.innerText = gameName; // 修改主遊戲標題
        const data = await fetchGetTwitchAPI(`https://api.twitch.tv/kraken/streams/?game=${gameName}`); // 再取得遊戲的頻道資訊
        const streansLimit = data.streams.length;
        for (let i = 0; streansLimit < 20 ? i < streansLimit : i < 20; i += 1) {
            const box = {
                streamer: data.streams[i].channel.name, // 實況主名稱
    ...
    async function init() { // 頁面初始化
        const data = await fetchGetTwitchAPI(top10gameHttps);
        const top1GameName = data.top[0].game.name;
        for (let i = 0; i < 5; i += 1) {
            liGroup[i].innerText = data.top[i].game.name; // 將前五名的遊戲名稱放入標籤中
            liGroup[i].title = data.top[i].game.name; // 滑鼠移上去時顯示文字
